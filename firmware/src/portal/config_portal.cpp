...
function ensureTiles(){
  ...
  while(grid.tiles.length<n){
    ...
    grid.tiles.push({id:"tile_"+i,label:"Tile "+(i+1),color:COLORS[i%COLORS.length],icon:"app",action:"app",action_id:"open_safari",target:{kind:"bundle",value:"com.apple.Safari"}});
  }
...
function render(){
  ...
  grid.tiles.forEach((t,idx)=>{
    ...
    const actionId=el("input",{value:t.action_id||"",placeholder:"open_vscode / volume_up"});
    actionId.oninput=e=>{t.action_id=e.target.value};
    ...
    const val=el("input",{value:(t.target&&t.target.value)||"",placeholder:"legacy hint (optional)"});
    function ensureActionId(){
      if(t.action_id&&t.action_id.length) return;
      if(t.action==="app") t.action_id="open_"+(t.id||"app");
      else t.action_id=t.action||"";
      actionId.value=t.action_id;
    }
    function syncTarget(){
      ensureActionId();
    ...
    action.onchange=e=>{t.action=e.target.value; if(t.action!=="app"){t.action_id=t.action; actionId.value=t.action_id;} syncTarget();};
    ...
    preset.onchange=e=>{
      ...
      t.action="app"; t.action_id="open_"+t.id; t.target={kind:"bundle",value:p.value};
      render();
    };
    ...
    card.appendChild(el("label",{text:"action_id"})); card.appendChild(actionId);
    card.appendChild(el("label",{text:"legacy target kind"})); card.appendChild(kind);
    card.appendChild(el("label",{text:"legacy bundle / path"})); card.appendChild(val);
