...
static void setTile(GridTile& t, const char* id, const char* label, uint32_t color, const char* icon,
                    TileAction action, AppTargetKind kind = AppTargetKind::None, const char* value = "") {
  memset(&t, 0, sizeof(t));
...
void gridConfigSetDefaults(GridConfig& cfg) {
  ...
  cfg.rev = 1;
  cfg.tile_count = 8;
  setTile(cfg.tiles[0], "vol_down", "Vol -", 0x475569, "vol_down", TileAction::VolumeDown);
  setTile(cfg.tiles[1], "mute", "Mute", 0x64748B, "mute", TileAction::Mute);
  setTile(cfg.tiles[2], "vol_up", "Vol +", 0x475569, "vol_up", TileAction::VolumeUp);
  setTile(cfg.tiles[3], "telegram", "Telegram", 0x229ED9, "telegram", TileAction::App,
          AppTargetKind::Bundle, "ru.keepcoder.Telegram");
  setTile(cfg.tiles[4], "gpt", "ChatGPT", 0x10A37F, "gpt", TileAction::App, AppTargetKind::Bundle,
          "com.openai.chat");
  setTile(cfg.tiles[5], "codex", "Codex", 0x0D8A6A, "codex", TileAction::App, AppTargetKind::Bundle,
          "com.openai.codex");
  setTile(cfg.tiles[6], "cursor", "Cursor", 0x6366F1, "cursor", TileAction::App, AppTargetKind::Bundle,
          "com.todesktop.230313mzl4w4u92");
  setTile(cfg.tiles[7], "iterm", "iTerm", 0x1F2937, "iterm", TileAction::App, AppTargetKind::Bundle,
...
bool gridConfigValidate(const GridConfig& cfg, char* err, size_t err_len) {
  ...
  for (uint8_t i = 0; i < cfg.tile_count; ++i) {
    ...
    if (t.action == TileAction::App) {
      if (t.app_kind == AppTargetKind::None || t.app_value[0] == '\0') {
        return fail("app tile missing target");
      }
      // Reject shell metacharacters — companion launches via execFile only.
