...
bool requiresBleHid(TileAction action) {
  return action == TileAction::Mute || action == TileAction::PlayPause ||
         action == TileAction::Next || action == TileAction::Previous;
}

// Launching an app is relayed over Wi-Fi to the macOS companion.
bool requiresCompanion(TileAction action) { return action == TileAction::App; }

bool tileAvailable(TileAction action) {
  if (requiresBleHid(action)) {
    return bleHidIsConnected();
  }
  if (requiresCompanion(action)) {
    return eventServerClientCount() > 0;
  }
  return true;
}
...
void layoutHeaderStatus() {
  // Right cluster: volume · BT · WS · Wi-Fi
  if (s_wifi) {
...
void updateConnStatus() {
  ...
  const uint8_t ws_clients = eventServerClientCount();
  ...
  if (s_ws) {
    lv_obj_set_style_img_recolor(
        s_ws, ws_clients > 0 ? lv_color_hex(0x4ADE80) : lv_color_hex(0x64748B), 0);
  }
...
void onTilePressed(lv_event_t* e) {
  ...
  if (!tileAvailable(tile.action)) {
    Serial.printf("[UI] tile %s disabled — %s not connected\n", tile.id,
                  requiresBleHid(tile.action) ? "BLE HID" : "companion");
    return;
  }
